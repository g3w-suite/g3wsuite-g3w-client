...
const ProjectsMenuComponent = require('gui/components/changemap/projectsmenu');
...
const floatbar = require('./components/floatbar/floatbar');
const viewport = require('./components/viewport/viewport');
const navbaritems = require('./components/navbar/navbaritems');
...
const ApplicationTemplate = function({ApplicationService}) {
  ...
  this._createTemplateConfig = function() {
    ...
    // get sdk componets
    const ContentsComponent = require('./components/viewport/contentsviewer');
    const CatalogComponent = require('gui/components/catalog/catalog');
    const SearchComponent = require('gui/components/search/search');
    const PrintComponent = require('gui/components/print/print');
    const MetadataComponent = require('gui/components/metadata/metadata');
    const ToolsComponent = require('gui/components/tools/tools');
    const MapComponent = require('gui/components/map/map');
    const QueryResultsComponent = require('gui/components/queryresults/queryresults');
    return {
      title: appTitle,
      placeholders: {
        navbar: {
...
ApplicationTemplate.fail = function({language='en', error }) {
  ...
  const compiledTemplate = Vue.compile(require('./500.html'));
  const app = new Vue({
    el: '#app',
    ...compiledTemplate,
    data: {
