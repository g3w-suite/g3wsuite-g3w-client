...
gulp.task('browserify', [], function() {
  let rebundle;
  let bundler = browserify(path.resolve(__dirname,'./src/index.js'), {
    basedir: "./",
    paths: [path.resolve(__dirname, "src"), path.resolve(__dirname, "config")],
    debug: !production,
    cache: {},
    packageCache: {}
  });
...
gulp.task('less',['fonts'], function () {
  ...
  return gulp.src([path.resolve(__dirname,'./src/assets/style/less/app.less'), path.join(pluginsLessFolder, 'plugin.less')])
    .pipe(concat('app.less'))
    .pipe(less({
      paths: [path.resolve(__dirname,'./src/assets/style/less'), pluginsLessFolder], // add paths where to search in @import
...
gulp.task('fonts', function () {
  return gulp.src(['!./src/**/node_modules/**/','./src/plugins/**/*.{eot,ttf,woff,woff2}','./src/assets/fonts/*.{eot,ttf,woff,woff2}'])
    .pipe(flatten())
    .pipe(gulp.dest(clientFolder+'/fonts/'))
});

gulp.task('images', function () {
  return gulp.src(['!./src/**/node_modules/**/', './src/assets/images/**/*.{png,jpg,gif,svg}','./src/plugins/**/*.{png,jpg,gif,svg}'])
    .pipe(flatten())
...
gulp.task('datatable-images',function () {
  if (!build_all) return;
  return gulp.src(path.resolve(__dirname, './src/assets/libraries/datatables/DataTables-1.10.16/images/*'))
    .pipe(flatten())
...
gulp.task('watch',function() {
  watch(['./src/assests/style/*.less'],
    prepareRunSequence('less','browser:reload')
  );
  watch(['./src/assests/style/skins/*.less'],
